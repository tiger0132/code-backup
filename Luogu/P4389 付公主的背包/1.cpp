...
const int N = 530035, P = 998244353, G = 3, GInv = 332748118;
...
static int r[N], lim = 1, l, gpow[N], ginvpow[N];
void ntt(int *a, int op) {
	for (int i = 0; i < lim; i++)
		if (i < r[i]) std::swap(a[i], a[r[i]]);
	for (int i = 1; i < lim; i <<= 1) {
		int gn = (op == 1 ? gpow : ginvpow)[i];
...
int main() {
	scanf("%d%d", &n, &m);
	for (int i = 1; i < 524288; i <<= 1) {
		gpow[i] = p(G, (P - 1) / (i << 1));
		ginvpow[i] = p(GInv, (P - 1) / (i << 1));
	}
	while (k <= m) k <<= 1;
