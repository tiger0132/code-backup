...
const int N = 5.3e5 + 535, P = 998244353, G = 3, sqrt2 = 116195171, isqrt2 = 557219762;
...
int main() {
	...
	n[ifac] = pw(n[fac], P - 2);
	for (int i = n - 1; i >= 1; i--) i[ifac] = (L)(i + 1)[ifac] * (i + 1) % P;

	g.resize(n);
	g[0] = 1;
	for (int i = 1; i < n; i++) {
		int x = (L)ifac[i] * pw(isqrt2, (L)i * i % (P - 1)) % P;
		g[i] = i & 1 ? P - x : x;
	}
	g.inv();
	for (int i = 0; i < n; i++) {
		g[i] = (L)g[i] * pw(sqrt2, (L)i * i % (P - 1)) % P;
	}
	g.ln();
	for (int i = 1; i < n; i++) out << (L)g[i] * fac[i] % P << '\n';
}