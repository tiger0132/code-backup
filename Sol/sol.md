...
首先将所有偶数位的 `A` 变成 `B`，`B` 变成 `A`。那么不能删除的就是 `AA` 和 `BB` 了。

一个字符串合法当且仅当 `A`、`B` 出现次数都不超过 $\frac n2$。

然后容斥，枚举一个超过 $\frac n2$ 的字符数量。~~CSP-S 2019 D2T1 警告~~

没了

# [P2] AGC012C [*2200]

考虑构造 $1,2,\ldots,k$ 的一个排列 $p$，然后让答案为 $p_1,p_2,\ldots,p_k,1,2,\ldots,k$。那么合法子序列数量就是 $p$ 中 LIS 的数量。

如果我们认为空串也是 LIS，那么每次在排列最后追加一个 $\max+1$ 可以让 LIS 数量 $\times2$，追加一个 $\min-1$ 可以让 LIS 数量 $+1$，然后直接进制分解。

# [P1] P3620 [*2200]

设 $dp_{i,j,0/1}$ 为前 $i$ 个，配 $j$ 对，最后两个有没有配对。
...
注意到每次 compress 对其它点的度数没有影响（除了重边的情况）

然后除了孤立环会剩下一个点以外，所有度数为 $0,2$ 的点都会被删掉。然后每次 compress 会让边数 $-1$。

# [P1] UVA1642 [*1600]

一个数和其它数一直做 $\gcd$ 最多只会变化 $\log$ 次。

然后记录每个值下标最靠前的出现位置，直接做就好了。

# [P1] CF1148F [*2500]

只能感性理解……

从高位到低位枚举。令当前枚举到的是位数 $i$，统计 $s$ 等于所有 $a_j$，满足 $msk_j$ 的 $\operatorname{lowbit}$ 是 $2^i$。
...
如果图有欧拉回路，那么可以交错地把欧拉回路上的边翻转，然后用自环平衡奇偶性。

所以问题就变成最少加几条边让图有欧拉回路。

# [P1] CF512D [*2900]

首先，环上的和环之间的肯定不会被删除。

那么可以将环之间的点作为根，然后在有根树上跑树形背包。具体地，设 $dp_{i,j}$ 为，以 $i$ 为根的子树中删 $j$ 个点的方案数。

如果是无根树，那么令其中的每一个叶子为根都跑一遍。每一个删 $k$ 个点的方案都会被统计 $n-k$ 次，其中 $n$ 是树的大小。

然后把所有的连通块都按照背包的方式合并就好了。

# [P1] P4585 [*2500]

注意到后缀修改可以转化成单点修改，然后做线段树分治。

然后并不需要回退操作，对每个点独立做就行了。

# [P1] P4983 [*2300]

其实这个式子就是 $\left(1+\sum_{i=1}^nx_i\right)^2$。然后 wqs 二分 + 斜率优化就好了。

# [P1] CF235C [*2700]

建出原串的 SAM，假设查询的串是 $T$。

考虑把 $T$ 倍长，设它为 $2T$。那么我们要求的就是 $2T$ 中所有长度为 $|T|$ 的子串的在 $S$ 中出现次数之和（去重）。
...
每次操作完之后，我们需要知道，这个字符串在 $S$ 中出现过的，最长的后缀对应在 SAM 上的节点。

设当前点为 $p$，我们来分类讨论：

- 对于插入操作：

	- 如果存在一条转移边 $p\xrightarrow{c}q$，那么返回 $q$。

	- 如果不存在，那么跳到 $\operatorname{link}(p)$ 继续尝试，直到跳回 $t_0$。

- 对于删除操作，我们考虑记录当前匹配的最长后缀的长度 $l$。

	- 如果 $l=\operatorname{len}(\operatorname{link}(p))$，那么返回 $\operatorname{link}(p)$。

	- 否则 $l\gets l-1$，返回 $p$。

# [P1] P2178 [*2500]

建反串 SAM，然后分别记录每个点所在子树的后缀点权值的最大最小、次大次小值。

# [P1] P3181 [*2200]

建出两个串的广义 SAM，设某个点 $x$ 对应的串在 $s_1$ 出现了 $a$ 次，$s_2$ 出现 $b$ 次，那么答案是 $|\operatorname{endpos}(x)|ab$。

# [P1] P4070 [*2100]

SAM 模板题，插入的时候跟着维护一下就好了。

# [P1] P1117 [*2600]

考虑求出所有的 `AA` 型串。

枚举一个长度 $l\in[1,\lfloor n/2\rfloor]$，在原串上隔 $l$ 个数放一个标记点。如果相邻两个标记点上的前缀 LCS 和后缀 LCP 有重叠，那么重叠部分都能作为断点。方案数差分维护。

# [P1] P1792 [*2000]

wqs 二分。

设 $f(i,0/1,0/1)$ 代表前 $i$ 个数，$i-1$ 选没选，$1$ 选没选的方案数，$g(i,0/1,0/1)$ 为最优方案选的数的个数（的最大值）。

显然有 $f(1,1,1)=a_1+k,f(1,1,0)=f(1,0,1)=-\infty,f(0,0,0)=0$。

对于每个 $i$，考虑如下转移：

- 选第 $i$ 个：
	- 如果 $1$ 没被选，$f(i,1,0)=f(i-1,0,0)+a_i+k$。
	- 如果 $1$ 被选了且 $i<n$，$f(i,1,1)=f(i-1,0,1)+a_i+k$
- 不选第 $i$ 个：
	- $[f(i,0,0/1),g(i,0,0/1)]=\max([f(i-1,0,0/1),g(i-1,0,0/1)],[f(i-1,1,0/1),g(i-1,1,0/1)])$。

然后答案是 $f(n,*,*)$ 里的最大值。

# [P1] CF484E [*2700]

考虑二分答案。设 $b_i=[a_i\ge mid]$，如果找到一个长度 $\ge k$ 的全 $1$ 子段，那么答案一定 $>mid$。然后可以用主席树维护每个 $mid$ 的线段树。

# [P2] CF963D [*2400]

因为**询问串长互不相同**，所以最多会有 $\sqrt{\sum m}$ 次询问。

然后每次询问暴力 $O(n)$ DFS 一遍 SAM，把所有匹配位置找出来排个序就好了。

# [P1] P4022 [*2400]

首先我们预处理 $mx_i$，代表 $i$ 结尾的串最长能匹配到的长度。

考虑二分 $L$。设 $f(i)$ 为前 $i$ 个字符，最后一段在 $i$ 处结尾时最长的覆盖长度。

然后显然有 $\displaystyle f(i)=\max_{i-mx_i\le j\le i-L}\{f(j)-j\}+i$。

然后注意到 $i-mx_i$ 是单调不减的，所以可以单调队列。

# [P2] P4081 [*2100]

建出广义 SAM，标记每个前缀属于哪个子串，然后 DFS 一遍，把只属于一个子串的点统计出来。