...
int qry(int x, int y) {
	...
	for (int tx, ty; (tx = x[top]) != (ty = y[top]);) {
		...
		if (tx[dep] > ty[dep])
			...
			printf("(2) QRY [%d, %d]\n", ty[dfn], y[dfn]),
				r = qry(1, ty[dfn], y[dfn]) + r, y = ty[par];
		printf("L: [%d %d %d %d]\n", l.s, l.la, l.ra, l.gss);
		printf("R: [%d %d %d %d]\n\n", r.s, r.la, r.ra, r.gss);
	}
	if (x[dep] > y[dep])
		printf("[1] QRY [%d, %d]\n", y[dfn], x[dfn]), l = qry(1, y[dfn], x[dfn]) + l;
	else
		printf("[2] QRY [%d, %d]\n", x[dfn], y[dfn]), r = qry(1, x[dfn], y[dfn]) + r;
	printf("L: [%d %d %d %d]\n", l.s, l.la, l.ra, l.gss);
	printf("R: [%d %d %d %d]\n\n", r.s, r.la, r.ra, r.gss);
	std::swap(l.la, l.ra);
	tag s = l + r;
...
int main() {
	...
	for (int i = 1; i <= n; i++) printf("%d%c", i[dfn], " \n"[i == n]);
	for (int i = 1; i <= n; i++) printf("%d%c", i[top], " \n"[i == n]);
	// prt(1);
	for (scanf("%d", &m); m--;) {
		scanf("%d%d%d", &op, &x, &y);
		if (op == 1)
